...
# ── 진단 (OOF 분석 — 학습 후 실행) ───────────────────────────────────────────
python dev/experiments/analyze.py            # seed=42 기준 (섹션 1~15 포함)
python dev/experiments/analyze.py --seed 777
python dev/experiments/analyze.py --seeds 42 777  # 2-seed OOF 비교
python dev/experiments/analyze.py --seed 42 --out-tag _stage2a_b120d97   # Stage 2-A 실험 분석
python dev/experiments/analyze.py --seed 42 --out-tag _stage2b_gated_v1  # Stage 2-B 분석
...
# ── Stage 2-A: soft label reweight ──────────────────────────────────────────
python dev/experiments/train.py --seed 42 --turn-boost 1.20 --jerk-decay 0.97 \
...
| Cand features | **CAND_DIM=14** (interaction) | +obs_acc_perp/par_match/perp_match/jerk_match |
...
| **2-seed 앙상블 (42+777), temp=2.0** | 50★ | — | 77.02% | 84.1% | ~0.6480 | **LB 0.6732** ✅ 신기록 (+0.0016 vs 0.6716) |
...
**[Phase 15: cand_feat physics interaction — LB 0.6776 신기록 +0.44pp]**

**아이디어**: "후보의 물리 가정이 실제 관측과 얼마나 맞는가"를 명시적 피처로 제공  
기존 Cross-Attention이 암묵적으로 학습해야 했던 관계를 per-candidate 피처로 직접 계산.

**구현 변경사항**

| 파일 | 변경 내용 |
|---|---|
| `config.py` | `CAND_FEAT_INTERACTION` 플래그 추가 (False=CAND_DIM 10, True=CAND_DIM 14) |
| `candidates.py` | `make_cand_features_gpu/make_cand_features`에 4개 interaction 피처 추가 |
| `model.py` | `CAND_DIM = 10 + (4 if CAND_FEAT_INTERACTION else 0)` 동적 계산 |

**추가 피처 4개 (CAND_DIM 10 → 14)**

| # | 피처 | 설명 |
|---|---|---|
| 10 | `obs_acc_perp_n` | 법선 가속도 크기 / speed (관측값, global) |
| 11 | `par_match` | `spec.par × obs_acc_par` (per-candidate interaction) |
| 12 | `perp_match` | `spec.perp × obs_acc_perp` (per-candidate interaction) |
| 13 | `jerk_match` | `spec.jerk × obs_jerk_par` (per-candidate interaction) |

**실험 결과 비교 (seed42 단독)**

| 실험 | OOF | std | B-group |
|---|---:|---:|---:|
| clip=True 단독 | 0.6487 | ±0.0069 | 45.0% ← 최악 |
| interaction + clip=True + Focal=0.05 | 0.6486 | — | 44.6% ← Focal 역효과 |
| interaction + clip=True | 0.6513 | ±0.0066 | 44.4% |
| **interaction + clip=none (채택)** | **0.6504** | **±0.0052** | **44.3%** ← B-group·std 최저 |

**최종 제출 (interaction + no-clip, 2-seed 42+777)**

| seed | OOF | B-group | Oracle rank mean |
|---:|---:|---:|---:|
| 42 | 0.6504 | 44.3% | 14.2 |
| 777 | 0.6501 | 44.2% | 13.5 |

→ **LB 0.6776** ✅ 전체 신기록 (+0.0044 vs 0.6732)

**현재 확정 Config (Phase 15)**

```python
CAND_FEAT_INTERACTION = True
CAND_FEAT_NORM_MODE   = "none"
FOCAL_LML_WEIGHT      = 0.0
LISTMLE_WEIGHT        = 0.10
PATIENCE              = 300
```
...
- B-group 44% 벽 미돌파 (목표 <43%) — 현재 approach의 한계
- C-group 23% miss — 2단계(LB 0.71+) 진입 조건
- 다음 방향: Track B (C-group 타겟 후보 추가 + case-aware bias) 또는 3rd seed 앙상블

---

### 2026-05-28 (2)

**[Phase 16 Track A-1: 3rd seed 탐색 결과]**

**단일 seed 결과**

| seed | OOF | std | B-group | C-group | Oracle rank mean | best temp | 판정 |
|---:|---:|---:|---:|---:|---:|---:|---|
| 42 (base) | 0.6504 | ±0.0052 | 44.3% | 23.0% | 14.2 | 2.0 | — |
| 777 (base) | 0.6501 | — | 44.2% | 23.0% | 13.5 | 2.0 | — |
| 999 | 0.6502 | ±0.0088 | 44.6% | 23.0% | 14.6 | 1.5 | ❌ 고분산·B악화 |
| **2026** | **0.6519** | **±0.0059** | **43.9%** | 23.0% | 13.8 | 1.0 | ✅ 유망 |
| **3407** | **0.6531** | **±0.0055** | **44.3%** | 23.0% | 13.8 | 1.0 | ✅ 유망 |

**멀티-seed OOF 앙상블 비교 (섹션 7, temp=1.0)**

| 조합 | models | Multi-seed OOF | vs 2-seed baseline |
|---|---:|---:|---:|
| 2-seed (42+777) | 10 | 0.6489 | baseline |
| 3-seed (42+777+3407) | 15 | 0.6524 | **+0.35pp** |
| 3-seed (42+777+2026) | 15 | **0.6526** | **+0.37pp** |
| 4-seed (42+777+2026+3407) | 20 | 0.6524 | +0.35pp |

**판정 및 제출 전략**

- **seed999**: 제외. std=0.0088(불안정), B-group 악화(44.6%), best_temp=1.5(로짓 불확실)
- **42+777+2026**: OOF 최고 (+0.37pp). 1순위 제출 후보
- **42+777+3407**: 0.02pp 차이로 2순위. 4-seed 추가해도 개선 없음
- **42+777+2026+3407 4-seed**: 3-seed 대비 개선 없음(동률) → 제출 우선순위 낮음

**제출 파일 생성 명령어**

```bash
# 1순위: 42+777+2026 (OOF 최고, B-group 개선)
python dev/experiments/predict.py --seeds 42 777 2026 --temp 2.0 \
  --out-name submission_phase16_42_777_2026_temp2_top10.csv

# 2순위: 42+777+3407
python dev/experiments/predict.py --seeds 42 777 3407 --temp 2.0 \
  --out-name submission_phase16_42_777_3407_temp2_top10.csv
```

**실제 LB 제출 결과 (Track A-1)**

| 제출 ID | 파일 | LB | vs 기준 |
|---:|---|---:|---:|
| 1457217 | submission_phase16_42_777_2026 | 0.6768 | −0.0008 |
| 1457218 | submission_phase16_42_777_3407 | 0.6772 | −0.0004 |
| (기준) | Phase 15 2-seed 42+777 | 0.6776 | — |

- OOF +0.37pp 개선에도 LB에서 모두 역행 → 3-seed 방향 종료

---

### 2026-05-28 (3)

**[Phase 16 Track A-2: LISTMLE_WEIGHT 재탐색 결과]**

**seed42 단독 실험**

| lml weight | seed42 OOF | vs base(0.6504) | 통과 기준 |
|---:|---:|---:|---|
| 0.05 | ~0.6499 | −0.05pp | ❌ |
| 0.10 | ~0.6504 | 0.00pp | baseline |
| **0.15** | **0.6509** | **+0.05pp** | ✅ (미미하지만 통과) |
| 0.20 | ~0.6497 | −0.07pp | ❌ |

**seed777 lml=0.15 학습 → 2-seed 앙상블 OOF**

| 구성 | OOF | std | 판정 |
|---|---:|---:|---|
| seed42 lml=0.15 단독 | 0.6509 | — | 단독 +0.05pp |
| seed777 lml=0.15 단독 | 0.6489 | ±0.0074 | Fold5=0.6360 약세 |
| **2-seed (42+777) lml=0.15** | **0.6489** | — | **기준(0.6489)과 동일** |

- seed777 lml=0.15가 약세(std 크고 Fold5 약함)로 앙상블 후 단독42 대비 −0.20pp 희석
- 2-seed 앙상블 OOF 0.6489 = 현행 lml=0.10 기준과 동일 → 개선 없음

**Track A-2 결론: LISTMLE_WEIGHT=0.10 유지 확정**

---

### Phase 16 Track A 전체 결론

| Track | 실험 | OOF 변화 | LB 결과 | 결론 |
|---|---|---:|---:|---|
| A-1 | 3rd seed (2026/3407) | +0.35~0.37pp | −0.0004~0.0008 **역행** | 종료 |
| A-2 | lml=0.15 2-seed | 0.00pp (기준과 동일) | 미제출 | 종료 |

- **Track A 전체 실패: LB 0.6776 (Phase 15) 유지**
- Track B 진입 조건 (LB ≥ 0.688) 미충족 → Track B 보류
...
| 항목 | 값 |
|---|---|
| **Best LB** | **0.6776** (Phase 15, 2-seed 42+777, interaction+no-clip) |
| **최종 목표** | **LB 0.71** |
| 현재 병목 | B-group 44% (selector ranking) + C-group 23% (oracle miss) |

**백업**

| 항목 | 경로 |
|---|---|
| submission | `archive/best_06776/submission_phase15_42_777_temp2_top10.csv` |
| seed42 checkpoints | `archive/best_06776/seed42/selector_fold*.pt` |
| seed777 checkpoints | `archive/best_06776/seed777/selector_fold*.pt` |
| config | `archive/best_06776/config.py` |
| 실행 명령 | `archive/best_06776/run_commands.txt` |

**Track A: 빠른 개선 탐색 ✅ 완료 — LB 개선 없음**

| 방향 | 결과 |
|---|---|
| A-1: 3rd seed 추가 (2026/3407) | OOF +0.37pp / LB −0.0004~0.0008 **역행** → 종료 |
| A-2: LISTMLE_WEIGHT=0.15 2-seed | OOF 동일(0.6489) → LISTMLE_WEIGHT=0.10 유지 확정 |

**Track A-1: 3rd seed 명령어**

```bash
# 작업 디렉토리: D:\Mosquito
# 현재 config.py 설정 그대로 사용 (CAND_FEAT_INTERACTION=True, no-clip, LML=0.10, PATIENCE=300)

python dev/experiments/train.py --seed 999
python dev/experiments/analyze.py --seed 999

python dev/experiments/train.py --seed 2026
python dev/experiments/analyze.py --seed 2026

python dev/experiments/train.py --seed 3407
python dev/experiments/analyze.py --seed 3407

# 단일 OOF ≥ 0.650 조건 통과 시 3-seed ensemble:
# python dev/experiments/predict_grid.py --seeds 42 777 999  --weights 0.33 0.33 0.34 --temp 2.0 --topk 10
# → 저장: outputs/grid/submission_grid_s42s777s999_w033_t20_top10.csv
# (파일명: submission_phase16_42_777_999_temp2_top10.csv 으로 수동 rename)
```

| 기준 | 판정 |
|---|---|
| 단일 OOF < 0.650 | 해당 seed 앙상블 제외 |
| 단일 OOF ≥ 0.650 | 42+777+해당 seed 3-seed ensemble OOF 확인 |
| 3-seed OOF < 2-seed OOF | 제출 취소 (seed123 역효과 교훈) |
| 3-seed OOF > 2-seed OOF | 제출 후보 등록 |

**Track A-2: LISTMLE_WEIGHT 재탐색 명령어**

```bash
# seed42 단독 실험 — 각각 별도 디렉토리에 저장 (기존 seed42 보존)
python dev/experiments/train.py --seed 42 --listmle-weight 0.05 --out-tag _lml05
python dev/experiments/analyze.py --seed 42 --out-tag _lml05

python dev/experiments/train.py --seed 42 --listmle-weight 0.10 --out-tag _lml10
python dev/experiments/analyze.py --seed 42 --out-tag _lml10

python dev/experiments/train.py --seed 42 --listmle-weight 0.15 --out-tag _lml15
python dev/experiments/analyze.py --seed 42 --out-tag _lml15

python dev/experiments/train.py --seed 42 --listmle-weight 0.20 --out-tag _lml20
python dev/experiments/analyze.py --seed 42 --out-tag _lml20

# 기준: OOF > 0.6504 (현재 seed42) + B-group < 44.3% + std 안정적
# 개선값 발견 시 seed777도 동일 설정 학습 후 2-seed ensemble 생성
```
...
| 단계 | 내용 |
|---|---|
| C-group 타겟 후보 추가 | miss-case 전용 5~8개 (Smart50 무작정 확대 금지) |
| C-group 라우터 | precision ≥ 0.80 threshold, OOF oracle miss label 학습 |
| C-group DL 모델 | raw trajectory → 직접 3D 회귀 (소형 Transformer/MLP) |

**금지 사항 (과거 실패 교훈)**

- GCN / GRU / AuxHead / RegMLP 재도입 금지
- routing 비율 13% 이상 허용 금지
- 라우터 없이 전체 샘플 DL 적용 금지
- Smart55/60 무작정 확대 금지

---

### 2026-05-29

**[Stage 1: Inference-level Calibration — 실패]**

목표: B-group ranking 개선 (재학습 없이)

핵심 발견:
- B-group(44.3%)의 jerk top1 선택 비율 **66.9%** — turn oracle(46.2%)을 jerk 후보가 누름
- dynamic topk (Rule A~D): 전 규칙 OOF 하락 (−0.006~−0.008) → A/B confidence 분포 동일하여 분류 불가
- soft reranking bias (perp_match×0.06 + turn_bias×0.02): seed42 +0.0006 OOF, 앙상블 −0.0002 → **폐기**
- 결론: jerk 편향은 training 문제, inference 보정으로 해결 불가

추가 도구:
- `dev/experiments/eval_bias.py` — bias 효과 빠른 검증
- `dev/experiments/predict_dynamic.py` — dynamic topk 제출 파이프라인
- `analyze.py` 섹션 9~13 추가 — B-group diagnostic, confidence feature, dynamic topk, soft bias, jerk 심층 분석

**[Stage 2-A: Soft Label Reweight — 부분 성공, LB 0.6774]**

방식: oracle family가 turn인 샘플에서 turn 후보 soft label 증폭 + jerk 감쇠

| 실험 | boost | decay | OOF | B-group | jerk_top1% |
|---|---|---|---:|---:|---:|
| b110 | 1.10 | 1.00 | 0.6508 | 45.0% | 66.4% |
| b115 | 1.15 | 1.00 | 0.6511 | 45.1% | 66.8% |
| b120 | 1.20 | 1.00 | 0.6503 | 45.1% | 66.4% |
| b125d95 | 1.25 | 0.95 | 0.6512 | 44.6% | 58.9% |
| **b120d97** | **1.20** | **0.97** | **0.6516** | **44.5%** | **58.4%** |

b120d97 결과: seed42 +0.0012 OOF, seed777 +0.0021 OOF, **LB 0.6774** (−0.0002 vs best 0.6776)
- jerk over-selection 8.5pp 감소, turn oracle rank 개선 (20.9→19.5), B_top10_RHit 개선 (0.8245→0.8287)
- B-group 크기 미달 (44.5% > 43.5%), LB 미개선 → Stage 2-A 종료
...
| 유형 | 수 | %_C | nearest |
|---|---:|---:|---|
| sharp_turn_C | 761 | 33.1% | turn |
| high_jerk_C | 754 | 32.8% | jerk |
| latency_C | 631 | 27.5% | latency |
| par_out_C | 152 | 6.6% | base/acc/frenet |

Gate 선정 — JT_q95 (obs_jerk ≥ 1.038 OR obs_acc_perp ≥ 0.626):
- activation=7.4%, C_prec=0.532, C_rec=0.172

Extra candidates v1 (6개, 기존 50 + 6 = 56):
| 후보 | 대응 유형 | 파라미터 |
|---|---|---|
| turn_p090 / turn_n090 | sharp_turn_C | perp=±0.90 |
| jerk_extreme_pos / neg | high_jerk_C | jerk=±1.20 |
| latency_s070 / l125 | latency_C | time_scale=0.70/1.25 |

구현 방식: Method A (mask -inf) — gate=False 샘플은 extra candidates logit 차단
